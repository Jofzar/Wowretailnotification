...
# Author: Jofzar  ; Based on ninthwalker/JumpY2K3  https://github.com/JumpY2k3/WoWNotifier             #
# Instructions: https://github.com/Jofzar/wownotificationretail        #
# Date: 04.02.23                                                       #
# Version: 1.3                                                         #
#########################################################################
...
## 1.3 
# - Changed it to be a wow retail base on https://github.com/JumpY2k3/WoWNotifier 
#########################################################################
...
$discordWebHook = "https://www.discord.com/api/webhooks/10XX019XXXXXX84680/LH_densBpNZQXXXXXXEXAMPLEtxjUD7jf3uATU7khWWpX"

## TELEGRAM ##
$telegram = $False
# Get the Token by creating a bot by messaging @BotFather
$telegramBotToken = "5995019717:AXXXXEXAMPLEbi8Vkeohipk"
...
# Then go to this url replacing <telegramBotToken> with your own Bots token and look for the chatID to use. 
$telegramChatID =  "584XXXXX29"
...
'@
    ...
    $proc = Get-Process wow | Select-Object -First 1
    if (-not ($proc)) {
...
function WoWNotifier {
    ...
    :check Do {
        ...
        if ($WoWAlert -like "*Avg*") {

            # Check for position in queue and ETA
            if ($WoWAlert -match '(?<=Avg.+)\d+') {
                $yourPos = $matches[0]
            }
            else {
                $yourPos = "Unknown"
            ...
            if ($queueCheck -eq "Yes") {
                ...
                if ([int]$last_send_time + [int]$queueCheckInterval*60 -lt [int]$time_now) {
                    ...
                    if ([int]$last_send_time -eq 0) {
                        $msg = "Started Queue Checker `n`r" + $msg
                    }
        ...
        elseif ($WoWAlert -like "*Enter*") {
            # set messages           
            $msg = "Solo Shuffle Queue has Popped! Have fun!"
            ...
            $label_status.ForeColor = "#7CFC00"
            $label_status.text = "Solo Shuffle Queue has Popped! Have fun!"
            $label_status.Refresh()
